   ...
   for uid in range(1, user_count+1):
       ...
       for idx, item, in enumerate(item_list):
           for idy in range(idx+1, len(item_list)):
               idx_iid, idx_score,tid_x = item_list[idx]
               idy_iid, idy_score,tid_y = item_list[idy]

               score = 1 - (abs(idx_score-idy_score) / (r_max - r_min))
...
if __name__ == '__main__':
    ...
    implicit_item_relations('./datasets/Epinions/')
    implicit_user_relations('./datasets/Epinions/')
